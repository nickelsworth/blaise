...
procedure TBlaiseLexer.SetData(const Data: Pointer; const Size: Integer);
begin
  FData := Data;
  FSize := Size;
  Reset;
...
begin
  ...
  Hash := 0;
  Repeat
    Hash := Byte(Hash + Ord(AnsiLowCaseLookup[D]));
    Inc(I);
    Inc(P);
    if I > FSize then
      break else
    ...
    begin
      ...
      if Length(S) = J then
        begin
          ...
          W := @FData[FPos];
          For G := 1 to J do
            if AnsiLowCaseLookup[V^] <> AnsiLowCaseLookup[W^] then
              begin
                R := False;
                break;
              end else
...
begin
  if FTokenLen <> Length(Token) then
    ...
    begin
      P := Pointer(FData);
      Inc(P, FTokenPos);
      Result := StrZMatchStr(P, Token, CaseSensitive);
      if Result and Skip then
        GetToken;
    end;
end;
