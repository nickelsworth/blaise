...
const
  ...
  UnitCopyright = 'Copyright (c) 1999-2003 by David Butler';
...
begin
  ...
  if L <> Length(EscSeq) then
    raise EStrInvalidArgument.Create('StrCharEscape: Invalid parameters');
  // Initialize lookup
...
begin
  if High(C) <> High(Replace) then
    raise EStrInvalidArgument.Create('StrCharUnescape: Invalid parameters');
  L := Length(EscPrefix);
  M := Length(S);
  if (L = 0) or (M <= L) then
    begin
