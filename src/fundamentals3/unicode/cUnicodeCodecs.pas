{                                                                              }
{                           Unicode codecs v3.07                               }
{                                                                              }
{             This unit is copyright © 2002-2003 by David J Butler             }
{                                                                              }
...
{                                                                              }
{ References:                                                                  }
{   ASCII and the Internet  http://www.faqs.org/rfcs/rfc20.html                }
{   IBM Code Pages          http://oss.software.ibm.com/icu/charset/           }
{                           roundtripIndex.html                                }
{   Character Codes         http://www.cs.tut.fi/~jkorpela/chars.html          }
{                                                                              }
{ Revision history:                                                            }
{   17/04/2002  0.01  Initial version.                                         }
{                     ISO8859-1 to ISO8859-15, Mac, Win1250-Win1252, UTF-8,    }
{                     UTF-16 support.                                          }
{   20/04/2002  0.02  Added EBCDIC-US support.                                 }
...
{   13/10/2003  3.07  Fixed bug in CP437 detected by Dieter Kohler.            }
{                     Added JIS-X0201 support.                                 }
{                                                                              }
...
function  JISX0201ToWideChar(const P: Char): WideChar;
...
function  WideCharToJISX0201(const Ch: WideChar): Char;
...
type
  ...
  TJISX0201Codec = class(AByteCodec)
  public
    class function GetUnicodeCodecType: TUnicodeCodecType; override;
    class function GetAliasCount: Integer; override;
    class function GetAliasByIndex(const Idx: Integer): String; override;
...
function  CreateUnicodeCodecByType(const CodecType: TUnicodeCodecType): AUnicodeCodec;
function  CreateUnicodeCodecByName(const Name: String): AUnicodeCodec;
...
const
  ...
  ISO8859_4Alias: Array[0..ISO8859_4Aliases - 1] of String = (
      ...
      'iso-ir-110', 'latin4', 'l4', 'csISOLatin4');
...
const
  ...
  ISO8859_9Alias: Array[0..ISO8859_9Aliases - 1] of String = (
      ...
      'iso-ir-148', 'latin5', 'l5', 'csISOLatin5');
...
const
  ...
  ISO8859_10Alias: Array[0..ISO8859_10Aliases - 1] of String = (
      ...
      'iso-ir-157', 'latin6', 'l6', 'csISOLatin6');
...
const
  ...
  ISO8859_13Alias: Array[0..ISO8859_13Aliases - 1] of String = (
      'ISO-8859-13', 'ISO_8859-13', 'latin7', 'l7', 'csISOLatin7');
...
const
  ...
  ISO8859_14Alias: Array[0..ISO8859_14Aliases - 1] of String = (
      ...
      'iso-ir-199', 'latin8', 'l8', 'iso-celtic', 'csISOLatin8');
...
const
  ...
  ISO8859_15Alias: Array[0..ISO8859_15Aliases - 1] of String = (
      ...
      'latin9', 'l9', 'latin0', 'l0', 'csISOLatin9');
...
{                                                                              }
{ References:                                                                  }
{   http://oss.software.ibm.com/cvs/icu/~checkout~/charset/data/ucm/           }
{          aix-JISX0201.1976_0-4.3.6.ucm                                       }
{                                                                              }
const
  JISX0201Aliases = 7;
  JISX0201Alias: Array[0..JISX0201Aliases - 1] of String = (
      'JISX0201', 'JISX0201.1976', 'JIS X 0201:1976', 'JIS X 0201:1997',
      'X0201', 'JIS C 6220', 'csHalfWidthKatakana');
...
begin
  Case Ord(P) of
    ...
    $A1..$DF  : Result := WideChar(Ord(P) + $FEC0);  // JIS X 0201:1976/1997 right half
    $00..$5B,
    $5D..$7D,
    $7F       : Result := WideChar(P);               // ASCII
  else
    raise EConvertError.Create('Invalid JIS-X0201 encoding');
  end;
end;
...
begin
  Case Ord(Ch) of
    $0000..$005B,
    $005D..$007D,
    $007F         : Result := Char(Ord(Ch));
    $00A5         : Result := #$5C;
    $00AF,
    $203E         : Result := #$7E;
    $FF61..$FF9F  : Result := Char(Ord(Ch) - $FEC0);
  else
    raise EConvertError.Create('Can not convert to JIS-X0201');
  end;
end;

class function TJISX0201Codec.GetUnicodeCodecType: TUnicodeCodecType;
begin
  Result := ucJISX0201;
end;

class function TJISX0201Codec.GetAliasCount: Integer;
begin
  Result := JISX0201Aliases;
end;

class function TJISX0201Codec.GetAliasByIndex(const Idx: Integer): String;
begin
  Result := JISX0201Alias[Idx];
end;

function TJISX0201Codec.DecodeChar(const P: Char): WideChar;
begin
  Result := JISX0201ToWideChar(P);
end;

function TJISX0201Codec.EncodeChar(const Ch: WideChar): Char;
begin
  Result := WideCharToJISX0201(Ch);
end;
...
begin
  C := GetUnicodeCodecClassByType(CodecType);
  if Assigned(C) then
    Result := C.Create
  else
    Result := nil;
...
var C : TUnicodeCodecClass;
begin
  C := GetUnicodeCodecClassByName(Name);
  if Assigned(C) then
    Result := C.Create
  else
    Result := nil;
