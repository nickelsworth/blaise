...
{   20/10/2003  3.06  Added support for Socks4 proxy.                          }
{                                                                              }
...
{                                                                              }
type
  TSocks4SocketProxy = class(ATCPClientSocketProxy)
  protected
    FDestHost     : String;
    FDestPort     : String;
...
begin
  if ErrorCode = 0 then
    SetResolved(Addr, FResolvedPort)
  else
...
begin
  ...
  if Assigned(FProxy) then
    DoResolve(lmAsync, FProxy.Host, FProxy.Port)
  else
...
constructor ATCPClientSocketProxyWithAuth.Create(const Host, Port: String;
    ...
    const Username: String; const Password: String;
    const Next: ATCPClientSocketProxy);
begin
...
{ TSocks4SocketProxy                                                           }
{                                                                              }
procedure TSocks4SocketProxy.DoNegotiate;
begin
  GetNextHost(FDestHost, FDestPort);
  DoResolve(lmAsync, FDestHost, FDestPort);
end;

procedure TSocks4SocketProxy.SetResolved(const Address: TInAddr; const Port: Word);
begin
...
procedure TSocks4SocketProxy.DoRequest;
var Msg : String;
begin
  Msg := Socks4ConnectRequest(FResolvedAddr, FResolvedPort, FUserID);
  FSocket.SendStr(Msg);
end;

procedure TSocks4SocketProxy.TriggerDataAvailable;
var Msg : TSocks4Message;
begin
  if FSocket.InBufferSize < Sizeof(TSocks4Message) then
    exit;
  FSocket.Read(Msg, Sizeof(TSocks4Message));
  if Msg.Version <> 4 then
    begin
      SetFailed(SocksErrorInvalidResponse, 'Invalid response from Socks server');
      exit;
    end;
  if Msg.Code <> SOCKS4_RESP_CODE_GRANTED then
    begin
