...
{                                                                              }
type
  TReaderEOLType  = (eolEOF, eolCR, eolLF, eolCRLF, eolLFCR);
  TReaderEOLTypes = Set of TReaderEOLType;

const
  DefaultReaderEOLTypes = [eolEOF, eolCR, eolLF, eolCRLF, eolLFCR];

type
  AReaderEx = class(AReader)
  protected
    procedure SkipLineTerminator(const EOLTypes: TReaderEOLTypes);
...
procedure AReaderEx.SkipLineTerminator(const EOLTypes: TReaderEOLTypes);
var C, D : Char;
begin
  if EOF then
    exit;
  ...
  if EOF then
    exit;
  if [eolCRLF, eolLFCR] * EOLTypes = [] then
    exit;
  D := Char(PeekByte);
...
begin
  ...
  Result := ExtractAll(NewLineChars^, True, MaxLineLength);
  if EOF then
    if eolEOF in EOLTypes then
      exit
    else
      RaiseReadError;
  SkipLineTerminator(EOLTypes);
end;

function AReaderEx.SkipLine(const MaxLineLength: Integer;
    const EOLTypes: TReaderEOLTypes): Boolean;
var I : Integer;
    NewLineChars : PCharSet;
begin
  FirstNewLineCharsFromEOLTypes(EOLTypes, NewLineChars);
